...
The databricks CLI bundles top level is defined by file `demo/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`demo/assets/model-workflow-asset.yml`)
 - batch inference workflow (`demo/assets/batch-inference-workflow-asset.yml`)
 - monitoring workflow (`demo/assets/monitoring-workflow-asset.yml`)
 - feature engineering workflow (`demo/assets/feature-engineering-workflow-asset.yml`)
 - model definition and experiment definition (`demo/assets/ml-artifacts-asset.yml`)
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML assets to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-demo-model     | /dev-demo-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest mlopsmain content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-demo-model | /staging-demo-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest mlopsrelease content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-demo-model    | /prod-demo-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the mlopsmain branch, upon making a PR, an integration test will be triggered and ML assets deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-demo-model    | /test-demo-experiment    |
...
You can open a PR (pull request) to modify ML code or the asset config against mlopsmain branch.
...
Upon merging a PR to the mlopsmain branch, the mlopsmain branch content will be deployed to the staging workspace with `staging` environment asset configurations.
...
2. Complete the `TODO` in `demo/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
```
Open a pull request to merge the pushed branch into the `mlopsmain` branch.
...
Once CI passes, merge the PR into the `mlopsmain` branch. This will deploy an initial set of Databricks assets to the staging workspace.
assets will be deployed to the prod workspace on pushing code to the `mlopsrelease` branch.
...
Following this, update the batch_inference_job base parameters in `demo/assets/batch-inference-workflow-asset.yml` to pass
...
To get started, open `demo/assets/batch-inference-workflow-asset.yml`.  The file contains the ML asset definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-demo-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-demo-batch-inference-job`
that runs the notebook under `demo/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `assets/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-demo-batch-inference-job`.
...
`test-demo-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-demo-batch-inference-job`.
...
After setting up CI/CD, PRs against the mlopsmain branch will trigger CI workflows to run unit tests, integration test and asset validation.
...
After merging a PR to the mlopsmain branch, continuous deployment automation will deploy the `staging` assets to the staging workspace.

When you about to cut a release, you can create and merge a PR to merge changes from mlopsmain to mlopsrelease. Continuous deployment automation will deploy `prod` assets to the prod workspace.
